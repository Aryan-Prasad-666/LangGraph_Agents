    ...
    def summarizer_node(state: CropPlanState) -> CropPlanState:
        try:
            ...
            default_plan = (
                ...
                f"For a {state['plan_duration']}-month crop plan in {state['location']}, plant heat-tolerant sorghum (Jowar) and moong (Jalgaon85) in early May, requiring intensive irrigation and potassium fertilizer (30 kg/ha). With the monsoon in July-August, plant kharif crops like rice (IR64-SRI with water management in the northwest), maize (HM-5), cotton (LC1136), and urad by late June or early July, incorporating two doses of potassium (20 kg/ha) and mulching. Harvest kharif crops in September-October and prepare soil for the rabi season with organic manure (10 t/ha), deep plowing, and potassium-rich compost (15 kg/ha). Throughout the period, monitor soil moisture, use drip irrigation, and implement integrated pest management (IPM) to ensure maximum yield."
            )
...
def generate_plan():
    try:
        ...
        plan_duration = int(data.get('plan_duration'))
        language = data.get('language')

        if not location or not plan_duration or not language:
            return jsonify({"error": "Missing required fields: location, plan_duration, or language"}), 400
